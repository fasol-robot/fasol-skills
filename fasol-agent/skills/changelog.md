...
## 2026-07-14 — Alert-autobuy buys now link to their alert in `/trades`

**Where:** [list-trades](list-trades.md) — the `source_kind` / `source_id`
fields on `GET /trades` rows.

Alert-autobuy BUYS (`tx_type: "aqb"`) used to come back with
`source_kind: null` and `source_id: null` — the entry buy wasn't linked to
the alert that caused it, only its TP/SL/trailing SELLS were. Now the buy
reports `source_kind: "alert"`, `source_id: "<alert_id>"` too, exactly like
those sells. One alert's whole autobuy cycle groups by `source_id`; `tx_type`
distinguishes the `aqb` entry from the `take_profit`/`stop_loss`/`trailing`
exits.

**What to do:**

- If you reconciled an alert's autobuy cycle by matching the entry buy from
  the sells' `source_id` (a workaround), you can drop it — the buy now
  carries the same `source_id`.
...
## 2026-07-14 — Agent trades now earn 80% fee cashback

**Where:** all agent-API trades — `POST /swap` and the fills of
agent-placed limit / TP / SL / trailing orders (`POST /orders`). No API
shape change; this is a cost-model change. See [swap](swap.md#cost-reminder).

Every transaction tagged to your key now earns **80% of the fasol platform
fee back** as claimable rewards, credited by a background worker to the
owner's reward balance (web `/rewards` page → withdraw). Mirrors the Chrome
extension cashback. Effective platform fee on agent trading drops to ~1/5 of
the sticker fee.

**What to do:**

- Nothing — it's automatic. But you MAY factor the lower effective fee into
  sizing / min-profit thresholds: the fee drag on a cycle is ~5× smaller
  than the raw `fasol_fee_sol` you see in [list-trades](list-trades.md).
